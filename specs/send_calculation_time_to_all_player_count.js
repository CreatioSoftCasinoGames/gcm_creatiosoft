...
var Robot = function(tokens, sendSpin){
  ...
  this.calculationTimeSent = false;
  ...
  this.pomelo.on("gameOver", function(data) {
    ...
    if(!this.calculationTimeSent) {
    	console.log('calculationTime broadcast didn\'t fired !!');
    } else {
    	console.log('calculationTime broadcast has been fired.');
    }
    this.calculationTimeSent = false;
  });
  this.pomelo.on("calculationTime", function(data) {
    //TODO: Just to check if this broadcast is coming or not
    this.calculationTimeSent = true;
    console.log(data);
  });
};
...
Robot.prototype = {
  ...
  checkKeyPresence: function(route, object, cb) {
    ...
    } else if (route == "joinMachine/gameProgress") {
