          ...
          if(data.startsIn == 0 && data.endsIn == 0) {
            ...
            if(that.sendSpin) {
              var spinInterval = setInterval(function() {
                ...
                redis.hgetall("jackpot_details", function(err, data){
                    var preMinJackpot     = parseInt(data.min_amount);
                    var preMajorJackpot   = parseInt(data.major_amount); 
                    var postMinJackpot    = preMinJackpot + Math.ceil(bet*(2/100));
                    var postMajorJackpot  = preMajorJackpot + Math.ceil(bet*(2/100));
                    ...
                    sockets[that.sessionId].request("slot.slotHandler.spinStatus", {coinsBet: bet, coinsWin: win}, function(data) {
                      that.checkKeyPresence("spinStatus", data, function(checkPass){
                        if(checkPass) {
                          // > Get jackpot values, update on backend and verify for correct data, 
                          // > timer  is given in order to let our code update values on backend 
                          // > then we proceed the verification process
                          redis.hgetall("jackpot_details", function(err, data){
                            if(parseInt(data.min_amount)!= postMinJackpot) {
                              that.caseFailed.reason = 'Wrong ! Min jackpot updated on redis!';
                              that.caseFailed.data = "";
                              console.log(JSON.stringify(that.caseFailed));
                            }
                            if(parseInt(data.major_amount) != postMajorJackpot) {
                              that.caseFailed.reason = 'Wrong ! Major jackpot updated on redis!';
                              that.caseFailed.data = "";
                              console.log(JSON.stringify(that.caseFailed));
                            }
                          });
