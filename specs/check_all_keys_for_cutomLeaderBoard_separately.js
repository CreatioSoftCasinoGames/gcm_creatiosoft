...
var Robot = function(tokens, sendSpin){
  ...
  this.pomelo.on("customLeaderBoard", function(data) {
    //TODO: Check all the existing keys present in this broadcast
    that.checkKeyPresence('customLeaderBoard', data, function(data) {});
  });
...
Robot.prototype = {
            ...
            console.log(JSON.stringify(that.caseFailed));
          	...
          	setTimeout(function(){
                ...
                console.log(JSON.stringify(that.caseFailed));
					    	...
					    	// console.log('\ngameStarted broadcast has been fired.');
					    }
  ...
  checkKeyPresence: function(route, object, cb) {
    ...
    } else if (route == 'customLeaderBoard') {
      expectedKeys =  [ 'awards', 'machineNumber', 'pot', 'remainingList', 'route', 'topThreeList', 'totalPlayers' ]
    }
    ...
    if(!_.isEqual(presentKeys, expectedKeys)) {
      ...
      // console.log('All expected keys are present in the route response - ' + route);
      cb(true);
