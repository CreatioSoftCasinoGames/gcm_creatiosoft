                	...
                	sockets[that.sessionId].request("slot.slotHandler.spinStatus", {coinsBet: bet, coinsWin: win}, function(data) {
	                  that.checkKeyPresence("spinStatus", data, function(checkPass){
	                    if(checkPass) {
	                    	...
	                    	setTimeout(function(){
	                    		redis.hgetall("user_profile:"+that.tokens.id, function(err, postDetails){
	                    			// console.log(preDetails.total_bet + " and " + bet + " and " + postDetails.total_bet);
	                    			// console.log(preDetails.coins_won + " and " + win + " and " + postDetails.coins_won);
	                    			// console.log(preDetails.coins_lost + " and " + ((bet > win) ? bet-win : 0) + " and " + postDetails.coins_lost);
	                    			// console.log(preDetails.total_spin + " and " + 1 + " and " + postDetails.total_spin);
	                    			var err = false;
	                    			if(parseInt(preDetails.total_bet) + parseInt(bet) != parseInt(postDetails.total_bet)) {
	                    				err = true;
	                    			} else if(parseInt(preDetails.coins_won) + parseInt(win) != parseInt(postDetails.coins_won)) {
	                    				err = true;
	                    			} else if(parseInt(preDetails.coins_lost) + ((bet > win) ? bet-win : 0) != parseInt(postDetails.coins_lost)) {
	                    				err = true;
	                    			} else if(parseInt(preDetails.total_spin) + 1 != parseInt(postDetails.total_spin)) {
	                    				err = true;
	                    			}
	                    			if(err) {
	                    				that.caseFailed.reason = 'Wrong data stored in redis for user profile - ' + JSON.stringify(preDetails);
												      that.caseFailed.data = JSON.stringify(postDetails);
												      console.log(JSON.stringify(that.caseFailed));
	                    			}
	                    		});
	                    	}, 1100)
